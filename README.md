...
Our project is made of two subprojects : mesh_converter and iga_solver. iga_solver depends of mesh_converter.
The project produces 2 libraries (libmesh_converter.so and libiga_solver.so) and 2 binaries linked against their
respective library (projArunner and projBrunner).
Packages for multiple platform should be produced : 1 single for iga_solver and 3 different for mesh_converter
(libs, devel, doc).
...
When compiling iga_solver independently we must have installed mesh_converter before, either from source or
from a package.
...
## Project structure and organisation
    ...
    │   │   └── mesh_converterConfig.cmake    # for subprojects interdependencies
    │   ├── CMakeLists.txt           # main CmakeLists.txt for mesh_converter
    │   ├── doc
    │   │   └── <Doxygen files and CMakeLists.txt>
    │   ├── include
    │   │   ├── CMakeLists.txt
    │   │   └── mesh_converter                # project subfolder to appear in includes (e.g. include <mesh_converter/World.h>)
    │   │       ├── CMakeLists.txt
    ...
    │   └── test
    │       ├── CMakeLists.txt
    │       └── testmesh_converter.cxx
    ├── iga_solver
    │   └── <same as mesh_converter>
    └── README

The headers and source files are split in each subproject. The headers are under a directory with
the name of the project. We want that users of our libraries write "include <mesh_converter/xxx.h>" and not
...
Important note : to be able to compile iga_solver alone, one must first make and install mesh_converter. 
Moreover, one must add to PATH and LD_LIBRARY_PATH the installation directory of mesh_converter.
